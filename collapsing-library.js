...
(async function () {
	...
	(() => {
		...
		const applyHeaderVisibility = (hide) => {
			...
			el.textContent = hide ? `
				.main-yourLibraryX-headerContent button[aria-label="Create"],
				.main-yourLibraryX-headerContent button[aria-label="Collapse Your Library"],
				.main-yourLibraryX-headerContent button[aria-label="Expand Your Library"] {
		...
		const SettingsPanel = ({ onClose, anchorRect }) => {
			...
			React.useEffect(() => {
				applyHeaderVisibility(hideHeader);
			}, [hideHeader]);
			// -- Drag logic --
		...
		async function main() {
			...
			const moveLibraryDiv = () => {
				...
				const moveLibraryDiv = () => {
					...
					if (!source || !target || source.parentElement === target) return;
					target.appendChild(source);
					applyHeaderVisibility(getSetting(KEYS.hideHeaderContent, true)); // ADD THIS
					log.info('Library control div moved into header.');
				};
			};
			moveLibraryDiv();
